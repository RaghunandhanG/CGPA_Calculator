...
def upload_file():
    uploaded_files = st.file_uploader("Choose your files", type=["pdf"], accept_multiple_files=True)
...
st.markdown(
    """
    <script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-8409482379247600" crossorigin="anonymous"></script>
    """,
...
if uploaded:
    if uploaded:
        ...
        if res :
            ...
            st.write(
                f'<h1> <span style="font-size:{additional_text_font_size};">Hi,</span><span style="font-size:{name_font_size};">{name}</span> <span style="font-size:{reg_no_font_size}; color:{reg_no_color};">{reg_no}</span></h1>',
                unsafe_allow_html=True)
